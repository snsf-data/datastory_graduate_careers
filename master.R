...
datastory_name <- "graduate_careers"

# Language-specific names, change! (used in URL: all lowercase, minus as 
# white-space and no special characters, no umlauts etc.)
datastory_name_de <- "emma-auf-dem-weg-zur-professorin"
datastory_name_en <- "emma-on-her-way-to-a-professorship"
datastory_name_fr <- "emma-sur-la-voie-du-professorat"

# English title and lead of the story (Mandatory, even if no EN version)
title_en <- "Emma – on her way to a professorship?" 
lead_en <- "Using Emma as an example, we profile the career path of an early career researcher whose PhD was financed by an SNSF project. To this end, we combined and analysed data from the Federal Statistical Office (FSO) and the SNSF." 
# German title and lead of the story (Mandatory, even if no DE version)
title_de <- "Emma – auf dem Weg zur Professorin?"
lead_de <- "Anhand von Emma zeigen wir exemplarisch den Karriereweg einer jungen Forschenden, die während ihres Doktorats über ein SNF-Projekt finanziert wurde. Dazu haben wir Daten des Bundesamtes für Statistik (BFS) mit den Daten des SNF verknüpft und analysiert."
# French title and lead of the story (Mandatory, even if no FR version)
title_fr <- "Emma – sur la voie du professorat ?"
lead_fr <- "L’exemple d’Emma nous permet d’illustrer le parcours professionnel d’une jeune chercheuse dont le doctorat a été financé par l’intermédiaire d’un projet du FNS. À cette fin, nous avons rapproché et analysé des données de l’Office fédéral de la statistique (OFS) et du FNS." 
...
publication_date <- "2021-06-28 00:10:00"
